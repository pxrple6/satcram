...
For Math, prioritize questions that test translation from words to equations, multi-step modeling, and interpretation — not one-step solves. Include at least 2 word-problem or context-modeling Math questions per set (styleTag like "equation-from-context", "consecutive-integers-model", "rate-model-setup", or "system-from-scenario"). Word problems must require the student to define variables and build an equation from descriptive language (e.g., consecutive integers, sum/difference relationships, rate × time, percent change). Distractors should reflect common setup mistakes such as wrong coefficients, swapped terms, or off-by-one consecutive offsets.
...
export async function generatePracticeWithOpenAI({ subject, topic, topics, recentStyleTags, sourceQuestion, sourceImage, similarToSubmitted = false }, apiKey) {
  ...
  const wordModelQuestions = questions?.filter((q) => q.subject === 'Math' && /equation-from-context|consecutive|model|scenario|word|translation|setup|rate-model|system-from/i.test(`${q.styleTag || ''} ${q.topic || ''}`)).length || 0
  if (!Array.isArray(questions) || questions.length !== 10 || new Set(styles).size !== 10 || styles.some((style) => !style || previouslyUsedStyles.has(style)) || !enoughTopics || !diverseRepresentations || (!similarMode && linearSolves > 2) || (!similarMode && Object.values(topicCounts).some((count) => count > 2)) || (!similarMode && mixed && (subjects.Math !== 4 || subjects.Reading !== 3 || subjects.Writing !== 3)) || (!similarMode && mixed && wordModelQuestions < 2) || questions.some((q) => !q.prompt || !(similarMode ? q.difficulty === 'Hard' : ['Medium', 'Hard'].includes(q.difficulty)) || !Number.isFinite(q.difficultyRating) || q.difficultyRating < (similarMode ? 4 : 3) || q.difficultyRating > 5 || !Array.isArray(q.choices) || q.choices.length !== 4 || !q.choices.includes(q.answer))) {
