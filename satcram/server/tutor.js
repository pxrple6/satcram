...
export async function tutorWithOpenAI({ messages, workReview = false, visualLesson = false }, apiKey) {
  ...
  const reviewInstruction = `\nWORK REVIEW OUTPUT: The FIRST attached image is the student's work. Any later attached image is the original question context. Return only JSON with {"feedback":string,"highlights":[{"x":number,"y":number,"width":number,"height":number,"label":string,"color":"red"|"amber"|"blue"|"green"|"purple"}],"concept":{"title":string,"visualType":"coordinate-plane"|"input-output"|"evidence-ladder"|"sentence-map","takeaway":string}|null}. Highlight coordinates MUST refer only to the first image, never the question-context image. When a visible step exists, return at least one highlight. Use up to four highlights: green for a valid setup, amber for a step to check, red for the first incorrect step, and blue for the next question the student must answer. Never supply the repair or final answer before the Socratic gate unlocks. Never pretend to see a step that is not visible.`
